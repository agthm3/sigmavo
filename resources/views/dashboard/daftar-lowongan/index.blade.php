...
@section('content')
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-gray-50"
         x-data="{ openDetailModal: false, selectedJob: null, activeLamarUrl: '' }">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 flex flex-col relative custom-scrollbar">
            ...
            <div class="bg-vokasi-dark px-4 py-8 lg:px-8 shadow-inner relative overflow-hidden">
                ...
                <div class="relative z-10 max-w-4xl mx-auto">
                    ...
                    <p class="text-vokasi-light mb-6 text-sm text-center md:text-left">Jelajahi berbagai posisi magang industri resmi dari mitra Vokasi UNHAS.</p>
                    ...
                    <form action="{{ route('dashboard-mahasiswa-daftar-lowongan') }}" method="GET" class="flex flex-col md:flex-row gap-2 bg-white p-2 rounded-xl shadow-lg">
                        <div class="flex-1 flex items-center bg-gray-50 rounded-lg border border-gray-200 px-3 py-2">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari posisi, perusahaan, atau kata kunci..." class="bg-transparent border-none outline-none w-full text-sm text-gray-700 focus:outline-none">
                        </div>
                        <div class="flex-1 flex items-center bg-gray-50 rounded-lg border border-gray-200 px-3 py-2">
            ...
            <div class="flex-1 p-4 lg:p-6 max-w-7xl mx-auto w-full">
                ...
                <div class="flex justify-between items-center mb-4">
                    <h4 class="font-bold text-gray-800 text-lg">Lowongan Magang Tersedia</h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                    ...
                    @php
                        $isApplied = in_array($job->id, $userPendaftaranIds);
                        $isFull = $job->kuota_terisi >= $job->kuota;
                        $sisaKuota = max(0, $job->kuota - $job->kuota_terisi);
                    @endphp
                    <div class="job-card bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden transition-all duration-300 flex flex-col h-full hover:border-vokasi-primary hover:shadow-md">
                        <div class="p-5 flex-1">
                            <div class="flex justify-between items-start mb-4">
                                <!-- Logo Perusahaan -->
                                <div class="w-12 h-12 rounded-lg bg-teal-50 border border-teal-100 flex items-center justify-center text-vokasi-primary font-bold text-base">
                            ...
                            <p class="text-vokasi-primary font-medium text-sm mb-3 line-clamp-1">{{ $job->perusahaan->nama_perusahaan ?? '-' }}</p>
                            
                            <div class="flex flex-wrap gap-2 mb-4">
                                <span class="bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-1 rounded">
                                    {{ $job->mode_kerja }}
                                </span>
                                <span class="bg-purple-50 text-purple-600 text-[10px] font-bold px-2 py-1 rounded line-clamp-1">
                                    {{ $job->prodi?->nama_prodi ?? 'Semua Prodi' }}
                                </span>
                            </div>
                            
                            <ul class="text-xs text-gray-500 space-y-2">
                                <li class="flex items-center"><i class="fas fa-map-marker-alt w-5 text-gray-400"></i> {{ Str::limit($job->perusahaan->alamat ?? '-', 25) }}</li>
                                <li class="flex items-center"><i class="fas fa-clock w-5 text-gray-400"></i> {{ $job->durasi }}</li>
                                <li class="flex items-center"><i class="fas fa-users w-5 text-gray-400"></i> Kuota: {{ $job->kuota_terisi }}/{{ $job->kuota }} (Tersisa {{ $sisaKuota }})</li>
                            </ul>
                        </div>

                        <div class="p-4 bg-gray-50 border-t border-gray-100 mt-auto flex items-center justify-between">
                            <span class="text-[11px] text-gray-400">
                                Batas: {{ $job->batas_pendaftaran ? $job->batas_pendaftaran->format('d M Y') : '-' }}
                            </span>
                            <button @click="selectedJob = {{ json_encode($job) }}; activeLamarUrl = '{{ route('dashboard-mahasiswa-daftar-lowongan-lamar', $job->id) }}'; openDetailModal = true" 
                                    class="text-vokasi-primary hover:text-vokasi-dark font-bold text-xs transition-colors">
            ...
            <div @click.away="openDetailModal = false" class="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-2xl overflow-hidden max-h-[90vh] flex flex-col" x-if="selectedJob">
                ...
                <div class="p-6 space-y-6 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-200 text-xs">
                        ...
                        <div>
                            <span class="text-gray-400 block font-semibold uppercase text-[10px]">Sisa Kuota</span>
                            <span class="font-bold text-emerald-600" x-text="(selectedJob?.kuota - selectedJob?.kuota_terisi) + ' Kursi Tersisa'"></span>
                        </div>
                        <div>
                ...
                <div class="p-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3 shrink-0">
                    ...
                    <form :action="activeLamarUrl" method="POST">
                        @csrf
                        <template x-if="selectedJob?.kuota_terisi < selectedJob?.kuota">
                        ...
                        <template x-if="selectedJob?.kuota_terisi >= selectedJob?.kuota">
                            <button type="button" disabled class="px-6 py-2 bg-gray-300 text-gray-500 rounded-xl font-bold text-xs cursor-not-allowed">
                                Kuota Sudah Penuh
                            </button>
                        </template>
                    </form>
